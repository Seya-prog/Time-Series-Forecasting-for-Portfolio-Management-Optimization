    ...
    for i, asset in enumerate(assets):
        if asset in data and not data[asset].empty:
            with metrics_cols[i]:
                ...
                total_return = (data[asset]['Close'].iloc[-1] / data[asset]['Close'].iloc[0] - 1) * 100
...
def show_forecasting(data, assets, forecast_months):
    ...
    with st.spinner(f"Generating forecast for {forecast_asset}..."):
        try:
            ...
            if len(prices) < 30:
                st.error(f"Insufficient data for {forecast_asset} (need at least 30 days)")
                return
            ...
            for i in range(forecast_periods):
                ...
                forecast_values.append(max(forecast_val, prices.min() * 0.5))  # Prevent negative prices
            ...
            with col1:
                ...
                last_date = prices.index[-1] if hasattr(prices.index[-1], 'date') else pd.Timestamp.now()
            ...
            st.info("This is a demo forecasting module. For production use, integrate with your trained ARIMA models.")
...
def show_portfolio_optimization(data, assets):
    ...
    with col1:
        ...
        target_return = st.slider("Target return (%)", 0.0, 50.0, 15.0) / 100
    
    with col2:
        st.subheader("Constraints")
        max_weight = st.slider("Max asset weight (%)", 10, 100, 50) / 100
        min_weight = st.slider("Min asset weight (%)", 0, 20, 0) / 100
...
def show_backtesting(data, assets):
    ...
    with st.spinner("Running backtest..."):
        try:
            ...
            benchmark_returns = returns_df.iloc[:, 0] if 'SPY' not in returns_df.columns else returns_df['SPY']
