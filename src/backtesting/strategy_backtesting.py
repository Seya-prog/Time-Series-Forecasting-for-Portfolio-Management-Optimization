...
class StrategyBacktester:
    ...
    def load_strategy_weights(self):
        ...
        try:
            ...
            with open(
                "results/portfolio_optimization_report.txt", "r", encoding="utf-8"
            ) as f:
            ...
            if weights_section:
                ...
                weights = {}
                for asset in self.assets:
                    weight_match = re.search(rf"{asset}:\s*([\d.]+)%", weights_text)
                    if weight_match:
    ...
    def simulate_portfolio_performance(
        self, weights: Dict, portfolio_name: str
    ) -> Dict:
        """
    ...
    def run_backtest(self):
        ...
        benchmark_performance = self.simulate_portfolio_performance(
            self.benchmark_weights, "Benchmark (60% SPY / 40% BND)"
        )
        ...
        strategy_performance = self.simulate_portfolio_performance(
            self.strategy_weights, "Optimized Strategy (Task 4)"
        )
    ...
    def plot_performance_comparison(
        ...
        plt.plot(
            ...
            label="Optimized Strategy (Task 4)",
            linewidth=2,
            color="red",
        )

        plt.title(
            "Portfolio Performance Comparison - Backtesting Results",
            fontsize=16,
            fontweight="bold",
        )
    ...
    def generate_backtest_report(self):
        ...
        report.append(f"   Annualized Return: {benchmark['annualized_return']:.2%}")
        ...
        report.append(f"   Annualized Return: {strategy['annualized_return']:.2%}")
        ...
        if positive_factors >= 2:
            ...
            recommendation = (
                "CAUTION: Consider strategy improvements before implementation"
            )
        else:
            conclusion = (
                "❌ STRATEGY UNDERPERFORMED - Model needs significant improvement"
            )
            recommendation = (
                "NOT RECOMMENDED: Stick with benchmark or improve forecasting model"
            )
