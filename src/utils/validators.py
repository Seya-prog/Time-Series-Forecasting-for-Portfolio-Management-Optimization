...
class DataValidator:
    ...
    @staticmethod
    def validate_price_data(data: pd.DataFrame, required_columns: List[str] = None) -> bool:
        ...
        if not high_missing.empty:
            logger.warning(f"Columns with >10% missing values: {high_missing.to_dict()}")
    ...
    def validate_returns(returns: pd.Series, max_daily_return: float = 0.5) -> bool:
        ...
        if extreme_returns.any():
            logger.warning(f"Found {extreme_returns.sum()} extreme returns (>{max_daily_return*100}%)")
    ...
    @staticmethod
    def validate_portfolio_weights(weights: Dict[str, float], tolerance: float = 1e-6) -> bool:
        ...
        if abs(total_weight - 1.0) > tolerance:
            raise ValueError(f"Weights sum to {total_weight}, not 1.0 (tolerance: {tolerance})")
...
class ModelValidator:
    ...
    def validate_forecast_inputs(data: pd.Series, min_observations: int = 100) -> bool:
        ...
        if len(data) < min_observations:
            raise ValueError(f"Insufficient data: {len(data)} < {min_observations} observations")
    ...
    @staticmethod
    def validate_forecast_outputs(predictions: np.ndarray, 
                                confidence_intervals: Optional[np.ndarray] = None) -> bool:
        ...
        if confidence_intervals is not None:
            if confidence_intervals.shape[0] != len(predictions):
                raise ValueError("Confidence intervals length doesn't match predictions")
...
def validate_business_rules(portfolio_weights: Dict[str, float], 
    ...
    if active_assets < min_assets:
        raise ValueError(f"Portfolio has only {active_assets} active positions, minimum {min_assets}")
