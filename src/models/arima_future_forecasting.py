...
class ARIMAFutureForecaster:
    ...
    def generate_arima_forecasts(self, months: int = 12) -> Dict:
        ...
        print(
            f"Forecast period: {future_dates[0].date()} to {future_dates[-1].date()}")
    ...
    def _generate_arima_forecast(self, periods: int) -> Dict:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"✅ ARIMA forecast generated with tightened confidence intervals")
                ...
                historical_returns = self.forecaster.data[self.forecaster.target_column].pct_change(
                ).dropna()
                ...
                print(
                    f"⚠️ Using basic prediction with approximate confidence intervals")
            ...
            if np.std(predictions) < 1.0:  # Very flat predictions
                ...
                avg_daily_return = np.clip(
                    avg_daily_return, -0.01, 0.01)  # Max 1% daily return

                # Apply linear trend instead of exponential
                trend_adjustment = np.arange(
                    periods) * avg_daily_return * predictions[0]
                ...
                print(
                    f"📈 Applied modest trend adjustment. New range: ${
                        np.min(predictions):.2f} - ${
                        np.max(predictions):.2f}")

        except Exception as e:
            print(f"⚠️ ARIMA future forecast failed: {e}")
            # Use trend-based forecast as fallback
            last_price = self.forecaster.data[self.forecaster.target_column].iloc[-1]
            historical_return = self.forecaster.data[self.forecaster.target_column].pct_change(
            ).mean()

            predictions = np.array(
                [last_price * (1 + historical_return) ** i for i in range(1, periods + 1)])
            std_error = np.std(
                self.forecaster.data[self.forecaster.target_column].pct_change().dropna())
            lower_ci = predictions - 1.96 * std_error * predictions
            upper_ci = predictions + 1.96 * std_error * predictions
            has_confidence_intervals = False
            print(f"⚠️ Using trend-based future forecast")
    ...
    def analyze_trends(self) -> Dict:
        ...
        print(f"  {risk_level}")
        print(
            f"  Price range: ${np.min(predictions):.2f} - ${np.max(predictions):.2f}")
    ...
    def identify_market_opportunities(self) -> Dict:
        ...
        if analysis['total_return'] > 10:
            opportunities.append(
                f"📈 Strong growth potential: {
                    analysis['total_return']:.1f}% expected return")
        elif analysis['total_return'] > 5:
            opportunities.append(
                f"📊 Moderate growth potential: {
                    analysis['total_return']:.1f}% expected return")

        if analysis['max_price'] > current_price * 1.2:
            opportunities.append(
                f"🎯 Significant upside potential: ${
                    analysis['max_price']:.2f} target")

        # Risk identification
        if analysis['total_return'] < -10:
            risks.append(
                f"📉 High downside risk: {
                    analysis['total_return']:.1f}% potential loss")
        elif analysis['total_return'] < -5:
            risks.append(
                f"⚠️ Moderate downside risk: {
                    analysis['total_return']:.1f}% potential loss")

        if analysis['volatility'] > 3:
            risks.append(
                f"🌊 High volatility: {
                    analysis['volatility']:.1f}% price swings expected")

        if analysis['min_price'] < current_price * 0.8:
            risks.append(
                f"🔻 Significant downside risk: ${
                    analysis['min_price']:.2f} potential low")
    ...
    def plot_arima_forecast(self, figsize: Tuple[int, int] = (15, 8)) -> None:
        ...
        if self.trend_analysis:
            ...
            textstr = f"Expected Return: {
                trend_info['total_return']:.1f}%\nVolatility: {
                trend_info['volatility']:.1f}%\n{
    ...
    def generate_comprehensive_report(self) -> str:
        ...
        report.append(
            f"Forecast Period: {
                self.future_forecasts['forecast_months']} months")
        report.append(f"Expected Return: {total_return:.1f}%")
        report.append(
            f"Trend Direction: {
                self.trend_analysis['trend_direction']}")
        ...
        if self.risk_analysis:
            ...
            report.append(
                f"Average Uncertainty: ±{
                    risk_data['avg_ci_width_pct']:.1f}%")
            report.append(
                f"Uncertainty Growth: {
                    risk_data['uncertainty_growth']:.1f}%")
        ...
        if total_return > 15:
            ...
            report.append(
                "BUY: Positive expected returns based on ARIMA analysis")
        elif total_return > -5:
            report.append("HOLD: Stable outlook with modest expectations")
        else:
            report.append(
                "CAUTION: Negative outlook, consider risk management")
...
def main():
    ...
    try:
        ...
        print(
            f"ARIMA forecast plot saved as 'results/figures/tesla_arima_future_forecast.png'")
