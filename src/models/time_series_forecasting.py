...
class TimeSeriesForecaster:
    ...
    def __init__(self, data: pd.DataFrame, target_column: str = 'Close'):
        ...
        if 'Date' in self.data.columns:
            ...
            self.data['Date'] = pd.to_datetime(
                self.data['Date']).dt.tz_localize(None)
        ...
        logger.info(
            f"Initialized forecaster with {len(self.data)} data points")

    def split_data(
            self, train_end_date: str = "2023-12-31") -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        logger.info(
            f"Training data: {
                ...
                self.train_data.index.min()} to {
                    self.train_data.index.max()})")
        logger.info(
            f"Testing data: {
                len(
                    self.test_data)} points ({
                self.test_data.index.min()} to {
        ...
        if not stationarity['is_stationary']:
            ...
            if stationarity_diff['is_stationary']:
                ...
                self.check_stationarity(
                    series_diff2, "Second Differenced Series")
                return series_diff2

        return series

    def fit_arima_model(self, auto_optimize: bool = True,
        ...
        if auto_optimize:
            ...
            try:
                ...
                print(
                    f"✅ Auto ARIMA completed with order: {
                        getattr(
                            model,
                            'order',
                            'unknown')}")
    ...
    def predict_arima(self) -> np.ndarray:
        ...
        try:
            ...
            predictions = np.full(n_periods,
                                  self.train_data[self.target_column].iloc[-1])
    ...
    def predict_lstm(self, steps: int = None) -> np.ndarray:
        ...
        last_sequence = self.train_data[self.target_column].tail(
        ...
        for _ in range(steps):
            ...
            X_pred = current_sequence[-sequence_length:].reshape(
                1, sequence_length, 1)
    ...
    def plot_predictions(self, figsize: Tuple[int, int] = (15, 10)) -> None:
        ...
        if not self.predictions:
            raise ValueError(
                "No predictions available. Run predict methods first.")
        ...
        for i, (model_name, pred_info) in enumerate(self.predictions.items()):
            ...
            ax.plot(test_dates[:len(predictions)],
                    predictions,
            ...
            if model_name == 'arima' and isinstance(
                    pred_info, dict) and 'confidence_interval' in pred_info:
                ci = pred_info['confidence_interval']
                ax.fill_between(test_dates[:len(ci)],
                                ci.iloc[:,
                                        0],
                                ci.iloc[:,
                                        1],
    ...
    def run_complete_analysis(
        ...
        self.calculate_metrics(
            actual_values[:len(lstm_pred)], lstm_pred, 'lstm')
        ...
        try:
            ...
            if 'lstm' in self.models:
                ...
                lstm_model_path = os.path.join(
                    models_dir, 'tesla_lstm_model.keras')
                ...
                lstm_metadata = {
                    ...
                    'train_end_date': str(
                        self.train_end_date) if hasattr(
                        self,
                        'train_end_date') else None}
                metadata_path = os.path.join(
                    models_dir, 'tesla_lstm_metadata.pkl')
            ...
            metadata_path = os.path.join(
                models_dir, 'tesla_forecaster_metadata.pkl')
            with open(metadata_path, 'wb') as f:
            ...
            print(
                f"🎯 All models successfully saved to '{models_dir}/' directory")
    ...
    def load_models(cls, models_dir: str = 'models') -> 'TimeSeriesForecaster':
        ...
        try:
            ...
            forecaster = cls(
                data=data, target_column=metadata['target_column'])

            # Set train_end_date if available
            if metadata.get('train_end_date'):
                forecaster.train_end_date = pd.to_datetime(
            ...
            lstm_model_path = os.path.join(
                models_dir, 'tesla_lstm_model.keras')
            scaler_path = os.path.join(models_dir, 'tesla_lstm_scaler.pkl')
            lstm_metadata_path = os.path.join(
                models_dir, 'tesla_lstm_metadata.pkl')
            ...
            print(
                f"🎯 All models successfully loaded from '{models_dir}/' directory")
            print(
                f"📅 Models saved on: {
                    metadata.get(
                        'save_timestamp',
...
def main():
    ...
    try:
        ...
        tesla_processed['Date'] = pd.to_datetime(
            tesla_processed['Date'], utc=True).dt.tz_localize(None)
        tesla_processed.set_index('Date', inplace=True)

        print(
            f"✅ Successfully loaded {
                len(tesla_processed)} Tesla processed data points")
        print(
            f"   Date range: {
                tesla_processed.index.min().date()} to {
                tesla_processed.index.max().date()}")
