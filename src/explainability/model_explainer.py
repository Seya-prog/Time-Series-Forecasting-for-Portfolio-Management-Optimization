...
class PortfolioExplainer:
    ...
    def _prepare_features(
        self, returns_data: pd.DataFrame, risk_factors: Optional[pd.DataFrame] = None
    ) -> pd.DataFrame:
        ...
        if len(returns_data.columns) > 1:
            ...
            for i, asset1 in enumerate(returns_data.columns):
                for j, asset2 in enumerate(returns_data.columns):
                    if i < j:
                        features[f"corr_{asset1}_{asset2}"] = [
        ...
        for _ in range(n_samples):
            ...
            expanded_features = pd.concat(
                [expanded_features, sample], ignore_index=True
            )
    ...
    def _train_surrogate_model(
        ...
        logger.info(
            f"Surrogate model trained with R² score: {model.score(features, expanded_target):.3f}"
        )
        return model

    def _generate_shap_explanations(
        self, model: RandomForestRegressor, features: pd.DataFrame
    ) -> Dict[str, Any]:
    ...
    def _create_explanation_plots(
        self, shap_results: Dict[str, Any], feature_importance: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    def _plot_shap_summary(self, shap_results: Dict[str, Any]) -> go.Figure:
        ...
        shap_values = shap_results["shap_values"]
        _ = shap_results["features"]  # features
        ...
        fig = go.Figure(
            go.Bar(
                x=top_shap_values, y=top_features, orientation="h", marker_color="coral"
            )
        )
...
class ForecastExplainer:
    ...
    def _explain_arima_forecast(
        self, data: pd.DataFrame, forecast: np.ndarray
    ) -> Dict[str, Any]:
        ...
        explanation = {
            ...
            "summary": self._generate_forecast_summary(
                features, trend_analysis, volatility_analysis
            ),
        }
    ...
    def _extract_time_series_features(self, prices: pd.Series) -> Dict[str, float]:
        ...
        features = {
            ...
            "current_price_vs_ma_63": (
                (prices.tail(63).mean() / prices.tail(252).mean()) - 1
                if len(prices) >= 252
                else 0
            ),
        }
    ...
    def _analyze_trend_components(self, prices: pd.Series) -> Dict[str, Any]:
        ...
        trend_analysis = {
            ...
            "trend_consistency": (
                returns.tail(21) > 0
            ).mean(),  # Fraction of positive days
    ...
    def _analyze_volatility_patterns(self, prices: pd.Series) -> Dict[str, Any]:
        ...
        volatility_analysis = {
            ...
            "volatility_trend": (
                "increasing" if vol_21d.iloc[-1] > vol_63d.iloc[-1] else "decreasing"
            ),
            "volatility_percentile": (
                vol_21d.iloc[-1] > vol_21d
            ).mean(),  # Current vol vs historical
            "extreme_moves_21d": (abs(returns.tail(21)) > 2 * returns.std()).sum(),
            "volatility_clustering": returns.tail(21).std()
            / returns.std(),  # Recent vs overall volatility
        }

        return volatility_analysis

    def _create_forecast_explanation_plots(
        self, prices: pd.Series, forecast: np.ndarray, features: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    def _plot_price_forecast(
        self, prices: pd.Series, forecast: np.ndarray
    ) -> go.Figure:
    ...
    def _plot_forecast_features(self, features: Dict[str, float]) -> go.Figure:
        ...
        fig = go.Figure(
            go.Bar(
                ...
                marker_color=[
                    "green" if v > 0 else "red" for v in relevant_features.values()
                ],
            )
        )
    ...
    def _plot_volatility_analysis(self, prices: pd.Series) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                x=prices.index, y=prices.values, name="Price", line=dict(color="blue")
            ),
            row=1,
            col=1,
        )
        ...
        fig.update_layout(
            title="Price and Volatility Analysis", height=600, showlegend=False
        )
