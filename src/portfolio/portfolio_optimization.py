...
class PortfolioOptimizer:
    ...
    def load_historical_data(self):
        ...
        try:
            ...
            print(f"✅ Loaded processed data from {data.index[0].date()} to {data.index[-1].date()}")
    ...
    def calculate_returns(self):
        ...
        print(f"✅ Calculated returns for {len(self.returns_data)} trading days")
        print(f"📈 Average daily returns:")
        for asset in self.assets:
            avg_return = self.returns_data[asset].mean()
            print(f"   {asset}: {avg_return:.4f} ({avg_return*252:.2%} annualized)")
        ...
        for asset in self.assets:
            if asset == 'TSLA':
                ...
                if tsla_forecast_return is not None:
                    ...
                    print(f"📊 TSLA: Using ARIMA forecast return: {tsla_forecast_return:.2%}")
                else:
                    # Fallback to historical if no forecast provided
                    self.expected_returns[asset] = historical_annual_returns[asset]
                    print(f"⚠️  TSLA: No ARIMA forecast provided, using historical: {historical_annual_returns[asset]:.2%}")
            else:
                # Use historical average for BND and SPY (Task 4 requirement)
                self.expected_returns[asset] = historical_annual_returns[asset]
                print(f"📈 {asset}: Using historical average return: {historical_annual_returns[asset]:.2%}")
    ...
    def generate_efficient_frontier(self, num_portfolios=10000):
        ...
        print(f"Generating efficient frontier with {num_portfolios} random portfolios...")
        ...
        for i in range(num_portfolios):
            ...
            portfolio_return, portfolio_vol, sharpe_ratio = self.portfolio_performance(weights)
    ...
    def find_optimal_portfolios(self):
        ...
        max_sharpe_return, max_sharpe_vol, max_sharpe_ratio = self.portfolio_performance(max_sharpe_weights)
        ...
        min_vol_return, min_vol_vol, min_vol_sharpe = self.portfolio_performance(min_vol_weights)
        ...
        print(f"   Weights: {dict(zip(self.assets, max_sharpe_weights.round(3)))}")
        
        print(f"\n📉 Minimum Volatility Portfolio:")
        print(f"   Return: {min_vol_return:.2%}")
        print(f"   Volatility: {min_vol_vol:.2%}")
        print(f"   Sharpe Ratio: {min_vol_sharpe:.3f}")
        print(f"   Weights: {dict(zip(self.assets, min_vol_weights.round(3)))}")
        
        return self.optimal_portfolios
    
    def plot_efficient_frontier(self, save_path='results/figures/efficient_frontier.png'):
        """
        Plot the efficient frontier with optimal portfolios marked
        """
        if self.efficient_frontier is None:
            raise ValueError("Efficient frontier not generated. Call generate_efficient_frontier() first.")
    ...
    def generate_portfolio_report(self, recommended_portfolio='max_sharpe'):
        ...
        if not hasattr(self, 'optimal_portfolios'):
            raise ValueError("Optimal portfolios not found. Call find_optimal_portfolios() first.")
        ...
        for i, asset1 in enumerate(self.assets):
            for j, asset2 in enumerate(self.assets):
                if i < j:
                    report.append(f"{asset1}-{asset2}: {corr_matrix.iloc[i,j]:.3f}")
        report.append("")
        ...
        if recommended_portfolio == 'max_sharpe':
            ...
            justification = "This portfolio minimizes risk and is suitable for conservative investors prioritizing capital preservation."
        ...
        report.append("-" * 40)
        report.append(f"Portfolio Beta (vs SPY): Calculated from correlations")
        report.append(f"Diversification Benefit: Achieved through multi-asset allocation")
        report.append(f"Maximum Drawdown Risk: Estimated at {recommended['volatility']*2:.1%} (2-sigma)")
        report.append("")
...
def main():
    ...
    try:
        ...
        try:
            ...
            import re
            expected_return_match = re.search(r'Expected Return: ([\d.-]+)%', report_content)
            
            if expected_return_match:
                tsla_arima_forecast = float(expected_return_match.group(1)) / 100
                print(f"📊 Extracted ARIMA forecast from Task 3: {tsla_arima_forecast:.2%}")
                
                # Handle edge case: if ARIMA predicts 0% return, use a small positive value for optimization
                if tsla_arima_forecast == 0.0:
                    print("⚠️  ARIMA predicts 0% return (stable prices). Using 2% for portfolio optimization.")
        ...
        optimizer.set_expected_returns(tsla_forecast_return=tsla_arima_forecast)
        ...
        report = optimizer.generate_portfolio_report(recommended_portfolio='max_sharpe')
