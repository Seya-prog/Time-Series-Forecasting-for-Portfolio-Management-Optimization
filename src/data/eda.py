...
class FinancialEDA:
    ...
    def plot_price_trends(self, save_path: Optional[str] = None) -> None:
        """Plot closing price trends for all symbols."""
        fig, axes = plt.subplots(
            len(self.symbols), 1, figsize=(15, 5 * len(self.symbols))
        )
        ...
        for i, symbol in enumerate(self.symbols):
            ...
            axes[i].plot(
                data.index, data[close_col], linewidth=2, label=f"{symbol} Close Price"
            )
            ...
            axes[i].set_title(
                f"{symbol} - Price Trends Over Time", fontsize=14, fontweight="bold"
            )
    ...
    def plot_returns_analysis(self, save_path: Optional[str] = None) -> None:
        ...
        fig, axes = plt.subplots(
            len(self.symbols), 2, figsize=(20, 5 * len(self.symbols))
        )
        if len(self.symbols) == 1:
        ...
        for i, symbol in enumerate(self.symbols):
            ...
            # Distribution plot
            axes[i, 1].hist(
                returns, bins=50, alpha=0.7, density=True, edgecolor="black"
            )
    ...
    def plot_volatility_analysis(self, save_path: Optional[str] = None) -> None:
        ...
        for i, symbol in enumerate(self.symbols):
            ...
            axes[i].plot(
                data.index, data["Volatility_5d"], alpha=0.7, label="5-day Volatility"
            )
            axes[i].plot(
                data.index, data["Volatility_20d"], alpha=0.8, label="20-day Volatility"
            )
...
def run_complete_analysis():
    ...
    collector = FinancialDataCollector(start_date="2015-07-01", end_date="2025-07-31")
    ...
    print(
        f"• Volatility ranking: {' > '.join(vol_data.sort_values(ascending=False).index.tolist())}"
    )

    # Return comparison
    ret_data = risk_metrics.set_index("Symbol")["Annual_Return"]
    print(
        f"• Return ranking: {' > '.join(ret_data.sort_values(ascending=False).index.tolist())}"
    )
    ...
    print(
        "✓ Data Collection: Successfully fetched 10+ years of data for TSLA, BND, SPY"
    )
    ...
    print(
        "✓ Statistical Analysis: Performed stationarity tests, calculated risk metrics"
    )
