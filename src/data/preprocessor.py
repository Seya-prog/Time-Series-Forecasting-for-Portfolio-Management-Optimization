...
class FinancialDataPreprocessor:
    ...
    def load_data(
        self, data_path: Optional[str] = None, data_dict: Optional[Dict] = None
        ...
        if missing_counts.sum() > 0:
            ...
            if "Volume" in data.columns:
                data["Volume"] = data["Volume"].fillna(
                    data["Volume"].rolling(window=5, center=True).median()
                )
...
def main():
    ...
    for symbol, results in stationarity.items():
        print(f"\n{symbol}:")
        print(
            f"  Prices - ADF p-value: {results['prices']['p_value']:.4f}, Stationary: {results['prices']['is_stationary']}"
        )
        print(
            f"  Returns - ADF p-value: {results['returns']['p_value']:.4f}, Stationary: {results['returns']['is_stationary']}"
        )
