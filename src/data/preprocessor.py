...
class FinancialDataPreprocessor:
    ...
    def load_data(self, data_path: str = None,
                  data_dict: Dict = None) -> Dict[str, pd.DataFrame]:
        """
        ...
        if data_dict:
            ...
            for symbol in combined_data['Symbol'].unique():
                self.data[symbol] = combined_data[combined_data['Symbol']
                                                  == symbol].copy()
    ...
    def clean_data(self) -> Dict[str, pd.DataFrame]:
        ...
        for symbol, data in self.data.items():
            ...
            # Remove duplicates
            cleaned_data = cleaned_data[~cleaned_data.index.duplicated(
                keep='first')]
    ...
    def _handle_missing_values(
        ...
        if missing_counts.sum() > 0:
            logger.warning(
                f"Found missing values in {symbol}: {missing_counts[missing_counts > 0].to_dict()}")
...
def main():
    ...
    for symbol, results in stationarity.items():
        print(f"\n{symbol}:")
        print(
            f"  Prices - ADF p-value: {
                results['prices']['p_value']:.4f}, Stationary: {
                results['prices']['is_stationary']}")
        print(
            f"  Returns - ADF p-value: {
                results['returns']['p_value']:.4f}, Stationary: {
                results['returns']['is_stationary']}")
